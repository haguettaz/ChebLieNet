...
def build_config(anisotropic: bool, coupled_sym: bool) -> dict:
    ...
    config = {
        ...
        "xi": 1.0 if not anisotropic else 460.0 if coupled_sym else 1e-4,
    }
...
def train(config=None):
    ...
    with wandb.init(config=config, project="gechebnet"):
        ...
        graph_lvl2 = SE2GEGraph(
            nx=48 if args.graph_pool else 96,
            ny=48 if args.graph_pool else 96,
            ntheta=config.ntheta,
            K=config.K,
        ...
        graph_lvl3 = SE2GEGraph(
            nx=24 if args.graph_pool else 96,
            ny=24 if args.graph_pool else 96,
            ntheta=config.ntheta,
            K=config.K,
